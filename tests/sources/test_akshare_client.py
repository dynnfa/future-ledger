...
ak: Any = akshare_client.ak  # type: ignore[attr-defined]
FIXED_NOW = datetime(2026, 5, 14, 8, 30, tzinfo=UTC)
...
def test_fetch_a_share_spot_returns_frame_and_metadata(monkeypatch: Any) -> None:
    ...
    monkeypatch.setattr(ak, "__version__", "1.17.0", raising=False)
...
def test_fetch_dividend_detail_records_empty_frame_source_error(monkeypatch: Any) -> None:
    ...
    monkeypatch.setattr(ak, "__version__", "1.17.0", raising=False)
    monkeypatch.setattr(ak, "stock_fhps_detail_em", lambda symbol: frame)
...
def test_fetch_dividend_detail_validates_symbol_before_live_call(
    ...
    monkeypatch.setattr(ak, "stock_fhps_detail_em", fake_detail)
...
def test_fetch_source_exception_returns_source_error_after_retries(
    ...
    monkeypatch.setattr(ak, "__version__", "1.17.0", raising=False)
    monkeypatch.setattr(ak, "stock_zh_a_spot_em", raising_spot)
